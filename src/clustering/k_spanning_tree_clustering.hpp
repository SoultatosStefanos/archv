...
namespace detail
{
    struct edge_hash
    {
        ...
        auto operator()(Edge e) const -> auto
        {
            ...
            return std::hash< type > {}(
                reinterpret_cast< type >(e.get_property()));
        }
    };
    ...
    void k_spanning_tree_clustering_impl(
        MutableGraph& g,
        unsigned k,
        ...
        WeightMap edge_weight)
    {
        ...
        boost::remove_edge_if(
            [&edges](auto e) { return !edges.contains(e); }, g);
    }
...
requires std::equality_comparable<
    typename boost::property_traits< WeightMap >::value_type >
inline void k_spanning_tree_clustering(
    MutableGraph& g, unsigned k, WeightMap edge_weight, MinimumSpanningTree mst)
{
